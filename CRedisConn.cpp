...
namespace RedisCpp {
// 错误描述。
const char* CRedisConn::_errDes[ERR_BOTTOM] = { "No error.",
		"NULL pointer , fatal error!", "Has no connection to the redis server.",
...
>>>>>>> 544fc1d5e7bf11ca920e84fad7c32bed15703b4e
...
void CRedisConn::init(const std::string &host, const uint16_t port,
...
bool CRedisConn::auth(const std::string& password) {
	...
	bool ret = false;
	_password = password;
	redisReply *reply = static_cast<redisReply *>(redisCommand(_redCtx,
			"AUTH %s", _password.c_str()));
...
redisReply* CRedisConn::redisCmd(const char *format, ...) {
	va_list ap;
	va_start(ap, format);
	redisReply* reply = static_cast<redisReply *>(redisvCommand(_redCtx, format,
			ap));
	va_end(ap);
	return reply;
}
...
bool CRedisConn::lpush(const std::string& key, const std::string& value,
		uint64_t& retval) throw (NullReplyException) {
	if (!_connected || !_redCtx) {
		_errStr = _errDes[ERR_NO_CONNECT];
		return false;
	}
...
<<<<<<< HEAD
bool CRedisConn::rpush(const std::string& key, const std::string& value,
		uint64_t& retval) {
	if (!_connected || !_redCtx) {
=======
...
<<<<<<< HEAD
bool CRedisConn::lindex(const std::string& key, int32_t index,
		std::string& value) {
	if (!_connected || !_redCtx) {
=======
...
>>>>>>> 544fc1d5e7bf11ca920e84fad7c32bed15703b4e
	...
	bool ret = false;
	redisReply *reply = redisCmd("LINSERT %s %s %s %s", key.c_str(),
			pos.c_str(), pivot.c_str(), value.c_str());
...
//////////////////hash方法/////////////////////////////
bool CRedisConn::hget(const std::string& key, const std::string& filed,
...
bool CRedisConn::hdel(const std::string& key, const std::string& filed,
	...
	bool ret = false;
	redisReply *reply = redisCmd("HDEL %s %s", key.c_str(), filed.c_str(),
			retval);
